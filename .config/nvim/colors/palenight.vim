...
call s:h("MatchParen", { "fg": s:black, "bg": s:white }) " The character under the cursor or just before it, if it is a paired bracket, and its match.
...
call s:h("Search", { "fg": s:black, "bg": s:red}) " Last search pattern highlighting (see 'hlsearch'). Also used for highlighting the current line in the quickfix window and similar items that need to stand out.
